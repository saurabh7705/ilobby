...
class EMailer
{
    ...
    */
	public function __construct()
	{
            ...
            $this->_myMailer->IsSMTP(); // send via SMTP
            $this->IsSMTP(); // send via SMTP
            $host = "email-smtp.us-west-2.amazonaws.com";
            $this->_myMailer->SMTPAuth = true; // turn on SMTP authentication
            $this->_myMailer->Username = "AKIAQWB4N2BDW53FP5ET"; // SMTP username
            $this->_myMailer->Password = "BL5K3t7ICfvzQrJqt6LjNVWMt+dOSec1O0Fc8QyoDOM5"; // SMTP password
            $this->_myMailer->Host=$host;
           $this->_myMailer->Port = 25;
           $this->_myMailer->FromName="Kommunity";
           $this->_myMailer->From="saurabh7705@gmail.com";
			// $this->_myMailer->IsAmazonSES();			
			// $this->_myMailer->AddAmazonSESKey("AKIAIUFRCHJ3JM5D2VNQ", "Ib49edmB+49Se/oexPLwjwbOIwOxtgV2ybKCR6js");			
			// $this->_myMailer->FromName = "Kommunity";
			// $this->_myMailer->From = "saurabh7705@gmail.com"; //Default
	}
	
	public function from($address) {
		$this->_myMailer->From = $address;
	}
    ...
    * @param string $layout
    */
	public function getView($view, $vars = array(), $layout = null, $link_params=array()) {
		...
		if($layout !== null)
			$body = $controller->renderInternal(Yii::getPathOfAlias($this->pathLayouts.'.'.$layout).'.php', array_merge($vars, array('content'=>$body)), true);
		$link_param_added_body = $body;
		$this->_myMailer->Body = $link_param_added_body;
	}

	public function getLayoutView($body, $vars, $layout, $link_params=array()) {
		$controller = new Controller('mail');
		$body = $controller->renderInternal(Yii::getPathOfAlias($this->pathLayouts.'.'.$layout).'.php', array_merge($vars, array('content'=>$body)), true);
		$link_param_added_body = $body;
		$this->_myMailer->Body = $link_param_added_body;
